...
{
	im1 = loadtexture("t90.png");
	im2 = loadtexture("t1.png");
	im3 = loadtexture("m1.jpg");
	im4 = loadtexture("m2.jpg");
}
...
window.onload = function main() 
{
    ...
    info_2p = initProgram(vssrc_2p, fssrc_2p, "OOOOOF");
	h1.textContent="апофеоз говна";
...
window.onkeydown = function kot(e)
{
	if(e.code=="Numpad8") 
	{
		const x=proj[ix]; proj[ix]=0;
		const y=proj[iy]; proj[iy]=0;
		const z=proj[iz]; proj[iz]=0;
		mat4.multiply(proj, proj, rotx0);
		proj[ix]=x;
		proj[iy]=y;
		proj[iz]=z;
	}
...
var scale = 1;
window.onwheel = function hotwheels(e) { proj[15] = dproj[15] += e.deltaY<0? -0.2: 0.2; }
...
class figure
{
	...
	render()
	{
		...
		gl.uniformMatrix4fv(info.viewloc, false, view);
...
const dproj = mat4.create();
function dumpval(v0, v1, dump)
{
	return v0*dump+v1*(1-dump);
}
function aim(m0, m1)
{
	for(var i=0; i<16; i++)
		m0[i]=dumpval(m0[i], m1[i], dump);
}
...
const rotx0 = mat4.create(), rotx1 = mat4.create();
const roty0 = mat4.create(), roty1 = mat4.create();
function basisinit()
{

	const scale=640;
	dproj[0]=scale/canv.width;
	dproj[5]=scale/canv.height;

	const rot = 0.02;
	mat4.rotateY(roty0, roty0, -rot);
	mat4.rotateY(roty1, roty1, +rot);

	mat4.rotateX(rotx0, rotx0, -rot);
	mat4.rotateX(rotx1, rotx1, +rot);
...
async function spaceinit()
{
	...
	//--------------primary--objects-----------
	const f0 = await loadfigure(src1, im1);
	figures.push(f0); f0.optimize_collider();
	f0.moveto(-16, 1, 0); f0.scale(16);
	lsrc.push(f0.pos); f0.pos[3]=0;
	f0.selfapply(movu);

	
	const f1 = new terrain(im3);
	figures.push(f1);
	f1.moveto(0, 0, 0); f1.scale(1);

	//--------------secondaty--objects-----------
	const f2 = await loadfigure(src1, im2);
	figures.push(f2); f2.optimize_collider();
	f2.moveto(-10, 1, 0); f2.scale(16);
	lsrc.push(f2.pos); f2.pos[3]=0;

	const f3 = await loadfigure(src1, im2);
	figures.push(f3); f3.optimize_collider();
	f3.moveto(0, 1, 0); f3.scale(16);
	lsrc.push(f3.pos); f3.pos[3]=0;
	
	const f4 = await loadfigure(src1, im2);
	figures.push(f4); f4.optimize_collider();
	f4.moveto(10, 1, 0); f4.scale(16);
